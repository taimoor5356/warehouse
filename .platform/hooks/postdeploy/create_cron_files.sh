...
# echo "5 * * * * webapp /usr/bin/php /var/app/current/artisan deactivate:packages 1>> /dev/null 2>&1" \
#   | sudo tee /etc/cron.d/artisan_scheduler

# echo "* * * * * webapp /usr/bin/php /var/app/current/artisan schedule:run 1>> /dev/null 2>&1" \
#  | sudo tee /etc/cron.d/artisan_scheduler

# echo "/usr/bin/nohup /usr/bin/php /var/app/current/artisan queue:work --timeout=10000000"

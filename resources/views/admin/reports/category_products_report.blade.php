                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header">
                                <div class="row w-100">
                                    <form action="{{ route('category_products_report_submit') }}" method="POST">
                                        @csrf
                                        <div class="row w-100">
                                                ...
                                                <label for="min">Categories</label>
                                                    <select name="category_id" id="category_id" class="form-control select2 category_id">
                                                        ...
                                                        @if($selected == 'all')
                                                            ...
                                                            @endforeach
                                                        @else
                                                            {{-- <option value="all"><span style="font-weight: bold">Show All</span></option> --}}
                                                            @foreach($categories as $category)
                                                                <option value="@if($selected == $category->id) {{ $selected }} @else {{ $category->id }} @endif" @if($selected == $category->id) selected @endif>@if($selected == $category->id) {{ $category->name }} @else {{ $category->name }} @endif</option>
                                                            @endforeach
                                                        @endif
                                                    </select>
                                            </div>
                                            <div class="form-group col-sm-12 col-md-3">
                                                ...
                                                <input type="text" class="form-control date_range" placeholder="Start Date" name="daterange" value="@isset($date1) {{ $date1 }} @else {{ $current_date }} @endisset - @isset($date2) {{ $date2 }} @else {{ $current_date }} @endisset">
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        var table = $('.single-product-table').DataTable({
            ...
            bDestroy: true,
            "ajax": {
            "url": "{{ route('show_single_category_product_report') }}",
            "data": {
    ...
    function format (d,row,tr,count,length) {
        ...
        var duration = $('#time_duration').val();
        var product_id = d.id;
        var html = '';
        $.ajax({

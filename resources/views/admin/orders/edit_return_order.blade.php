        ...
        <div class="content-header row">
            <div class="content-header-left col-md-9 col-12 mb-2">
                <div class="row breadcrumbs-top">
                    <div class="col-12">
                        <h2 class="content-header-title float-start mb-0">Edit Return Order</h2>
                        <div class="breadcrumb-wrapper">
                            <ol class="breadcrumb">
                                ...
                                <li class="breadcrumb-item"><a href="/order_return">Returned Reports</a>
                                </li>
                                <li class="breadcrumb-item active">Edit Return Order
                                </li>
                            </ol>
                        </div>
                    </div>
            ...
            <div class="col-md-12">
                <form id="orderForm" enctype="multipart/form-data">
                    <div class="row invoice-add">
                                        ...
                                        <div class="col-md-3 mb-lg-1 col-bill-to ps-0">
                                            ...
                                            <div class="invoice-customer">
                                                <select name="customer" id="customer" class="form-select select2" required>
                                                    ...
                                                    <option value="{{ $returnOrder->customer_id }}" selected>
                                                        {{ $returnOrder->customer->customer_name }}</option>
                                                </select>
                                                @error('customer')
                                ...
                                <div class="row">
                                    <div class="col-12">
                                        <div class="card">
                                            ...
                                            <div class="table-height" style="overflow-y: hidden; overflow-x: scroll;">
                                                <table class="table data-table table-bordered order_table" id="order_table"
                                                    style="padding-bottom: 190px; margin-bottom: 200px">
                                                    ...
                                                    <tbody>
                                                        @isset ($returnOrder->order_return_details[0])
                                                            @php
                                                                $orderReturnDetail = $returnOrder->order_return_details[0];
                                                            @endphp
                                                            <tr>
                                                                <td>
                                                                    <select name="brand[]" class="form-control brand select2"
                                                                        data-order-no="0">
                                                                        <option value="">Select Brand</option>
                                                                        @foreach ($brands as $brand)
                                                                            <option value="{{ $brand->id }}" @if ($orderReturnDetail->brand_id == $brand->id) selected @endif>{{ $brand->brand }}</option>
                                                                        @endforeach
                                                                    </select>
                                                                    <div id="brand-error" class="text-danger font-weight-bold">
                                                                    </div>
                                                                    <small class="text-danger d-none"
                                                                        id="brand_required">Required</small>
                                                                </td>
                                                                <td>
                                                                    <select name="item_status[]"
                                                                        class="form-control item_returned_satus">
                                                                        <option value="0">-Item Status-</option>
                                                                        <option value="1" @if($orderReturnDetail->item_status == 1) selected @endif>Return</option>
                                                                        <option value="2" @if($orderReturnDetail->item_status == 2) selected @endif>Damaged</option>
                                                                        <option value="3" @if($orderReturnDetail->item_status == 3) selected @endif>Opened</option>
                                                                        <option value="4" @if($orderReturnDetail->item_status == 4) selected @endif>Invalid Address</option>
                                                                    </select>
                                                                    <span
                                                                        class="item_status_not_selected_error d-none text-danger">Required</span>
                                                                </td>
                                                                <td>
                                                                    <input type="text" class="form-control order_number"
                                                                        name="order_number[]" value="{{ $orderReturnDetail->order_number }}" placeholder="Enter Order Number">
                                                                    <span
                                                                        class="order_number_not_selected_error d-none text-danger">Required</span>
                                                                </td>
                                                                <td>
                                                                    <input type="text" class="form-control name"
                                                                        name="name[]" value="{{ $orderReturnDetail->name }}" placeholder="Enter Name">
                                                                    <span
                                                                        class="name_not_selected_error d-none text-danger">Required</span>
                                                                </td>
                                                                <td>
                                                                    <input type="text" class="form-control state"
                                                                        name="state[]" value="{{ $orderReturnDetail->state }}" placeholder="Enter Enter State">
                                                                    <span
                                                                        class="state_not_selected_error d-none text-danger">Required</span>
                                                                </td>
                                                                <td>
                                                                    <select name="product_id0[]"
                                                                        class="form-control product_id select2"
                                                                        multiple="multiple" data-order-no="0">
                                                                        @php
                                                                            $brandProducts = \DB::table('customer_has_products')
                                                                                            ->join('products', 'customer_has_products.product_id', '=', 'products.id')
                                                                                            ->where('customer_id', $returnOrder->customer_id)
                                                                                            ->where('brand_id', $orderReturnDetail->brand_id)
                                                                                            ->select(['products.id as id', 'products.name as name'])
                                                                                            ->get();
                                                                        @endphp
                                                                        <option value="">-Select Product-</option>
                                                                        @php
                                                                            $orderedProducts = $returnOrder->order_return_details->pluck('product_id')->toArray();
                                                                        @endphp
                                                                        @foreach ($brandProducts as $product)
                                                                            <option value="{{ $product->id }}" @if (in_array($product->id, $orderedProducts)) selected="selected" @endif>{{ $product->name }}</option>
                                                                        @endforeach
                                                                    </select>
                                                                    <div class="product-error text-danger d-none">No Product Found.</div>
                                                                    <span
                                                                        class="product_required_error d-none text-danger">Required</span>
                                                                </td>
                                                                <td class="products_qty">
                                                                    <div class="input_fields">
                                                                        @php
                                                                            $orderedProducts2 = $returnOrder->order_return_details;
                                                                        @endphp
                                                                        @foreach ($orderedProducts2 as $key => $product)
                                                                            @php
                                                                                $productData = \App\AdminModels\Products::with('inventory')->where('id', $product->product_id)->first();
                                                                            @endphp
                                                                            <input type="number" class="form-control deduction_qty returnQuantity appended_classes" data-product-no="{{$key}}" data-price{{$key}}="{{ $productData->price }}" data-qty{{$key}}="{{ $productData->inventory->qty }}" name="return_qty0[]" placeholder="0" value="{{ $product->qty }}">
                                                                            <input type="hidden" class="form-control total_amount_field{{$key}} appended_classes" data-product-no="{{$key}}" value="{{ $productData->price }}" name="deducted_price0[]">
                                                                            <input type="hidden" class="form-control remaining_qty{{$key}} appended_classes" data-product-no="{{$key}}" name="remaining_qty0[]" value="{{ $productData->inventory->qty }}">
                                                                        @endforeach
                                                                    </div>
                                                                </td>
                                                                <td>
                                                                    ...
                                                                    <span
                                                                        class="description_not_selected_error d-none text-danger">Required</span>
                                                                </td>
                                                                {{-- <td>
                                                                    <button
                                                                        class="m-auto btn-primary add-row border-0 rounded-circle shadow"
                        ...
                        <input type="hidden" id="return-order-id" value="{{ $id }}">
...
<script type="text/javascript">
            ...
            $.ajax({
                ...
                success: function(result) {
                    if (result.status == "success") {
                        ...
                        result.data.forEach(element => {
                            brandOptions += "<option value='" + element.id + "'>" +
                                element
                                .brand + "</option>"
                        });
                        $('.brand-error').html('');
            ...
            $('.brand').each(function() {
                ...
                if (_this.val() != '') {
                    ...
                    if (brandOrderStatus.val() != '0') {
                        brandOrderStatus.closest('tr').find('.item_status_not_selected_error')
                            .addClass('d-none');
                        if (brandOrderProduct.val() != '') {
                            brandOrderProduct.closest('tr').find('.product_required_error')
                                .addClass('d-none');
                        } else {
                            brandOrderProduct.closest('tr').find('.product_required_error')
                                .removeClass('d-none');
    ...
    function submitForm() {
        ...
        var url = "{{ route('update_return_order', ':id') }}";
        url = url.replace(':id', $('#return-order-id').val());
        $.ajaxSetup({
            headers: {
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.status == true) {
                    alert(response.msg);
                    window.location.href = "/order_return";
                } else {
                    alert(response.msg);
                }
            }
            ...
            success: function(response) {
                ...
                if (response.length > 0) {
                    ...
                    response.forEach(element => {
                        options += "<option image='" + element.products.image + "' value='" +
                            element.product_id + "'>" + element.products.name + "</option>";
                    });
            ...
            success: function(response) {
                ...
                for (let index = 1; index <= 10; index++) {
                    html += `
                            <tr>
                                <td>
                                    <select name="brand[]" class="form-control brand select2" data-order-no="` +
                        index + `">
                                        <option value="">Select Brand</option>`;
                    html += brandOptions;
                    html += `</select>
                                ...
                                <td>
                                    <select name="product_id` + index +
                        `[]" class="form-control product_id select2" multiple="multiple" value="" data-order-no="` +
                        index + `">
                                         ` + options +
                        ` 
                                    </select>
    ...
    function formatState(state) {
        ...
        var $state = $(
            '<span><img src="' + baseUrl + image + '" class="img-flag" width="35" height="45" /> ' + state.text +
            '</span>'
        );
        return $state;
    };
